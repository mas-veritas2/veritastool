...
class ModelContainer(object):
    ...
    def __init__(self, y_true=None, p_grp=None, model_type='classification', model_name='auto', y_pred=None, y_prob=None, y_train=None, protected_features_cols=None, train_op_name="fit",
        ...
        if self.p_grp is not None:
            ...
            if protected_features_cols is None and x_test is not None:
                if type(self.x_test) == pd.DataFrame:                                                
                    ...
                    self.err.push('type_error', var_name='x_test', given=str(type(self.x_test)), expected=str(pd.DataFrame),
        ...
        self._input_validation_lookup = {
            "y_true": [(NoneType, list, np.ndarray, pd.Series), ],
        ...
        if self.p_grp != None:
            self._input_validation_lookup.update({"p_grp": [(dict,), check_p_grp], \
    ...
    def check_protected_columns(self):     
        ...
        if type(self.p_var) != list:
            ...
            err_.append(['length_error', "protected_features_cols and x_test", "None for both", "not both are None when p_grp is provided"])
        ...
        if self.y_train is not None:
            ...
            if type(self.x_train) == pd.DataFrame:
                ...
                if x_train_rows != train_row_count:
                    err_.append(['length_error', "x_train, y_train rows", "x_train rows " + str(x_train_rows), "y_train rows " + str(train_row_count)])
    ...
    def check_classes(self):
        ...
        if not issubclass(self.model_object.__class__, ModelWrapper) and not hasattr(self.model_object, 'classes_') and self.model_object is not None:
            if isinstance(self.y_prob, pd.DataFrame) and self.y_prob.shape[1] > 2:
        ...
        if self.model_object is not None and self.y_prob is not None and self.y_prob.ndim == 2 and self.y_prob.shape[1] > 2:
        ...
        if self.model_object is not None and self.y_true is not None and set(self.model_object.classes_) != set(self.y_true):
            err_.append(['value_error', "classes_", str(self.model_object.classes_), "labels in classes_ to be consistent with y_true"])

        if self.model_object is not None and self.y_pred is not None and set(self.model_object.classes_) != set(self.y_pred):
