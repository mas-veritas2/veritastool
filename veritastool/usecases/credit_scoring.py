...
class CreditScoring(Fairness, Transparency):
    ...
    def __init__(self, model_params, fair_threshold, perf_metric_name = "balanced_acc", fair_metric_name = "auto", fair_concern = "eligible", fair_priority = "benefit", fair_impact = "normal", fair_metric_type = 'difference', num_applicants = None, base_default_rate = None, fairness_metric_value_input = {}, tran_index = [1], tran_max_sample = 1, tran_pdp_feature = [], tran_pdp_target=None, tran_max_display = 10):
        """
        Parameters
        ----------
        model_params: list
            ...
            if self.model_params[0].y_pred is not None:
                ...
                for curr_p_var in self.model_params[0].p_var :
                    if self._rejection_inference_flag[curr_p_var] == True :
                        ...
                        tp_p, fp_p, tn_p, fn_p, tp_u, fp_u, tn_u, fn_u = self._get_confusion_matrix_optimized(y_true=y_true_reshape, y_pred=y_pred_reshape, sample_weight=self.model_params[0].sample_weight, curr_p_var = curr_p_var, feature_mask = self.feature_mask)
