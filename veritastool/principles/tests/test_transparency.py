...
cre_sco_obj1= CreditScoring(model_params = [container1], fair_threshold = 80, fair_concern = "eligible", \
                           ...
                           tran_row_num = [20,40], tran_max_sample = 1000, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 5,tran_processed_data = x_train, tran_processed_label = [y_train])
...
container1 = ModelContainer(y_true, p_grp, model_type, model_name, y_pred, y_prob, y_train, x_train=None, \
                           x_test=x_test, model_object=model_object, up_grp=up_grp)
cre_sco_obj1= CreditScoring(model_params = [container1], fair_threshold = 80, fair_concern = "eligible", \
                           fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", fair_metric_name = 'disparate_impact', fair_metric_type = 'difference',\
                           tran_row_num = [20,40], tran_max_sample = 1000, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 5,tran_processed_data = x_train, tran_processed_label = None)
assert cre_sco_obj1.tran_input_features['check_input_flag']==2

container1 = ModelContainer(y_true, p_grp, model_type, model_name, y_pred, y_prob, y_train=None, x_train=x_train, \
                           x_test=x_test, model_object=model_object, up_grp=up_grp)
cre_sco_obj1= CreditScoring(model_params = [container1], fair_threshold = 80, fair_concern = "eligible", \
                           fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", fair_metric_name = 'disparate_impact', fair_metric_type = 'difference',\
                           tran_row_num = [20,40], tran_max_sample = 1000, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 5,tran_processed_data = None, tran_processed_label = [y_train])
assert cre_sco_obj1.tran_input_features['check_input_flag']==2
...
def test_check_tran_row_num():
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                        fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                        tran_row_num=2, tran_max_sample = 100, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                        fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                        tran_row_num=['a',2], tran_max_sample = 100, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                        fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                        tran_row_num=[50000], tran_max_sample = 100, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                        fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                        tran_row_num=[1,2,-500], tran_max_sample = 100, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg

    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                        fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                        tran_row_num=pd.Series(np.array([1,20.0,50.2])), tran_max_sample = 100, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
...
def test_check_tran_max_sample():
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 50000, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = [25], tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 1000.56, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
...
def test_check_tran_pdp_feature():
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 1000, tran_pdp_feature = 'abc', tran_max_display = 10)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 1000, tran_pdp_feature = ['LimitBal'], tran_max_display = 10)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 1000, tran_pdp_feature = ["LIMIT_BAL","a","b","c"], tran_max_display = 10)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 1000, tran_pdp_feature = [1,"LIMIT_BAL"], tran_max_display = 10)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
...
def test_check_tran_pdp_target():
    ...
    try:
        with pytest.raises(Exception) as toolkit_exit:
            cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_pdp_target = 5, tran_max_display = 10)
    except:
        assert True
...
def test_check_tran_max_display():
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 1000, tran_pdp_feature = ["LIMIT_BAL"], tran_max_display = -9)
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 1000, tran_pdp_feature = ["LIMIT_BAL"], tran_max_display = 'top5')
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg

    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 1000, tran_pdp_feature = ["LIMIT_BAL"], tran_max_display = 50)
    assert cre_sco_obj.tran_max_display == 23

    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 1000, tran_pdp_feature = ["LIMIT_BAL"], tran_max_display = 0)
    assert cre_sco_obj.tran_max_display == 23

    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                            tran_row_num=[20,40], tran_max_sample = 1000, tran_pdp_feature = ["LIMIT_BAL"], tran_max_display = 15.4)
...
def test_check_tran_processed_data():
    ...
    with pytest.raises(Exception) as toolkit_exit:
        ...
        cre_sco_obj1= CreditScoring(model_params = [container1], fair_threshold = 80, fair_concern = "eligible", \
                            fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", fair_metric_name = 'disparate_impact', fair_metric_type = 'difference',\
                            tran_row_num = [20,40], tran_max_sample = 1000, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 5,tran_processed_data = [], tran_processed_label = [y_train])
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        ...
        cre_sco_obj1= CreditScoring(model_params = [container1], fair_threshold = 80, fair_concern = "eligible", \
                           fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", fair_metric_name = 'disparate_impact', fair_metric_type = 'difference',\
                           tran_row_num = [20,40], tran_max_sample = 1000, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 5,tran_processed_data = x_train, tran_processed_label = {})
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        ...
        cre_sco_obj1= CreditScoring(model_params = [container1], fair_threshold = 80, fair_concern = "eligible", \
                           fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", fair_metric_name = 'disparate_impact', fair_metric_type = 'difference',\
                           tran_row_num = [20,40], tran_max_sample = 1000, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 5,tran_processed_data = x_train, tran_processed_label = [1])
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg
    ...
    with pytest.raises(Exception) as toolkit_exit:
        ...
        cre_sco_obj1= CreditScoring(model_params = [container1], fair_threshold = 80, fair_concern = "eligible", \
                           fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", fair_metric_name = 'disparate_impact', fair_metric_type = 'difference',\
                           tran_row_num = [20,40], tran_max_sample = 100, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 5,tran_processed_data = x_train[:500], tran_processed_label = [y_train[:1000]])
    assert toolkit_exit.type == MyError
    assert toolkit_exit.value.message == msg

    msg = "[value_error]: tran_processed_label: given 1, expected length equal to length of model params : 2 at _tran_check_input()\n"
    with pytest.raises(Exception) as toolkit_exit:
        container_rej1 = ModelContainer(y_true = y_true_rej, y_pred = y_pred_rej, y_prob = y_prob_rej, y_train= None, \
                               p_grp = p_grp_rej, x_train = None,  x_test = x_test_rej, \
                               model_object = model_object_rej,  model_name = model_name_rej, model_type = model_type_rej,\
                               pos_label=['TR', 'CR'], neg_label=['TN', 'CN'])
...
def test_data_sampling():    
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                        fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                        tran_row_num=[20,40], tran_max_sample = 100, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 16)
    ...
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                        fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                        tran_row_num=[20,40], tran_max_sample = 0.2, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
    ...
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                        fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                        tran_row_num=[20,40], tran_max_sample = 500, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
    ...
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                        fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                        tran_row_num=[20,40], tran_max_sample = 1, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
...
def test_local():
    ...
    cm_uplift_obj1 = CustomerMarketing(model_params = [container_rej1, container_prop1], fair_threshold = 80, \
                                  fair_concern = "eligible", fair_priority = "benefit", fair_impact = "significant", \
                                  perf_metric_name = "expected_profit", fair_metric_name="auto", revenue = PROFIT_RESPOND, \
                                  treatment_cost = COST_TREATMENT, tran_row_num=[5,3], tran_max_sample=1000, \
                                  tran_pdp_feature= ['age','income'], tran_pdp_target='CR', tran_max_display = 6, \
                                  tran_processed_data = x_train_rej[:1000], tran_processed_label = [y_train_rej[:1000],y_train_prop[:1000]])
    cm_uplift_obj1.explain(model_num = 2)
    assert cm_uplift_obj1.model_params[1].model_object.classes_[cm_uplift_obj1.tran_results['model_list'][1]['plot']['class_index'][3]] == 'CN'
    assert cm_uplift_obj1.model_params[1].model_object.classes_[cm_uplift_obj1.tran_results['model_list'][1]['plot']['class_index'][5]] == 'CR'

def test_compute_partial_dependence():
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                           fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                           tran_row_num=[20,40], tran_max_sample = 1000, tran_max_display = 10)
    cre_sco_obj._data_prep()
    ...
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                           fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                           tran_row_num=[20,40], tran_max_sample = 1000,tran_pdp_feature=['AGE'], tran_max_display = 10)
    cre_sco_obj._data_prep()
    cre_sco_obj._compute_partial_dependence()
    ...
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                           fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                           tran_row_num=[20,40], tran_max_sample = 1000, tran_pdp_feature=['MARRIAGE','SEX','LIMIT_BAL','PAY_1'], tran_max_display = 10)
    cre_sco_obj._data_prep()
    cre_sco_obj._compute_partial_dependence()
...
def test_compute_permutation_importance():
    ...
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                           fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                           tran_row_num=[20,40], tran_max_sample = 1000, tran_max_display = 20)
    cre_sco_obj._data_prep()
    cre_sco_obj._compute_permutation_importance()
...
def test_data_prep():
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                    fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                    tran_row_num=[20,40], tran_max_sample = 100, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10)
    cre_sco_obj._data_prep()
    ...
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                    fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                    tran_row_num=[20,40], tran_max_sample = 100, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 5,
...
def test_explain():
    ...
    cre_sco_obj= CreditScoring(model_params = [container], fair_threshold = 80, fair_concern = "eligible", \
                        fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", \
                        tran_row_num=[20,40], tran_max_sample = 50, tran_pdp_feature = ['LIMIT_BAL'], tran_max_display = 10,
                        tran_features = ['MARRIAGE','SEX','LIMIT_BAL','PAY_1'])   
    ...
    cm_uplift_obj = CustomerMarketing(model_params = [container_rej, container_prop], fair_threshold = 80, \
                                  ...
                                  perf_metric_name = "expected_profit", fair_metric_name="auto", revenue = PROFIT_RESPOND, \
                                  treatment_cost =COST_TREATMENT, tran_row_num=[20,40], tran_max_sample=1000, \
                                  tran_pdp_feature= ['age','income'], tran_pdp_target='CR', tran_max_display = 6,tran_features=['age','income'])
    cm_uplift_obj._data_prep(model_num=1)
    cm_uplift_obj.explain(local_row_num=52)
    assert 52 in list(cm_uplift_obj.tran_results['model_list'][1]['plot']['local_plot'].keys())
    ...
    assert 1 not in list(cm_uplift_obj.tran_results['model_list'][1]['plot']['local_plot'].keys())
...
def test_tran_compile():
    ...
    cm_uplift_obj = CustomerMarketing(model_params = [container_rej, container_prop], fair_threshold = 80, \
                                  fair_concern = "eligible", fair_priority = "benefit", fair_impact = "significant", \
                                  perf_metric_name = "expected_profit", fair_metric_name="auto", revenue = PROFIT_RESPOND, \
                                  treatment_cost =COST_TREATMENT, tran_row_num=[20,8000.9], tran_max_sample=500, \
                                  tran_pdp_feature= ['age','income'], tran_pdp_target='CR', tran_max_display = 4)
    cm_uplift_obj._tran_compile()
    ...
    container1 = ModelContainer(y_true, p_grp, model_type, model_name, y_pred, y_prob, y_train, x_train=x_train, \
                                x_test=x_test, model_object=None, up_grp=up_grp)
    cre_sco_obj1= CreditScoring(model_params = [container1], fair_threshold = 80, fair_concern = "eligible", \
                           fair_priority = "benefit", fair_impact = "normal", perf_metric_name="accuracy", fair_metric_name = 'disparate_impact', fair_metric_type = 'difference',\
