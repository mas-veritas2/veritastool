...
def test_local():
    ...
    assert round(cre_sco_obj.tran_results['model_list'][0]['local_interpretability'][0]['feature_info'][1]['shap'],2)==-0.07
...
def test_compute_permutation_importance():
    ...
    cre_sco_obj._data_prep()
    cre_sco_obj._compute_permutation_importance()
    assert len(cre_sco_obj.tran_results['permutation_score'])==10
    ...
    cre_sco_obj._data_prep()
    cre_sco_obj._compute_permutation_importance()
    assert len(cre_sco_obj.tran_results['permutation_score'])==20
    assert cre_sco_obj.tran_results['permutation_score']!=None

    #testing for regression
    file = "veritas-toolkit/veritastool/examples/data/regression_dict.pickle" 
    input_file = open(file, "rb")
    ...
    assert list(np.round_(np.array(base_reg_obj.permutation_importance['diff']),6)) == [9093.108145, 2173.331941, 823.94601, 125.229173, 50.07561, 48.964686]
    assert list(base_reg_obj.permutation_importance['feature']) == ['smoker', 'age', 'bmi', 'children', 'sex', 'region']
...
def test_tran_compile():
    ...
    assert cm_uplift_obj.tran_flag[0]['total']==True
    assert cm_uplift_obj.tran_flag[1]['total']==True