...
def test_artifact():
    ...
    assert clf_obj.artifact['fairness']['features']['isforeign']['tradeoff']['fair'].shape == clf_obj.artifact['fairness']['features']['isforeign']['tradeoff']['perf'].shape
    assert clf_obj.array_size == clf_obj.artifact['fairness']['perf_dynamic']['threshold'].shape[0]
    assert clf_obj.array_size == len(clf_obj.artifact['fairness']['perf_dynamic']['perf'])
    assert clf_obj.array_size == len(clf_obj.artifact['fairness']['perf_dynamic']['selection_rate'])
...
def test_fairness_metric_value_input_check():
    ...
    assert clf_obj.fairness_metric_value_input == None
...
def test_compile():
    ...
    assert clf_obj.evaluate_status_cali == True
    assert clf_obj.evaluate_status_perf_dynamics == True
    assert clf_obj.tradeoff_status == 1
    assert clf_obj.feature_imp_status == 1
    assert clf_obj.feature_imp_status_loo == True
    assert clf_obj.feature_imp_status_corr == True
...
def test_tradeoff():
    assert round(clf_obj.tradeoff_obj.result['isforeign']['max_perf_point'][0],3) == 0.3
...
#             return test_preds
    ...
    #Create Model Container
    container = ModelContainer(y_true,  p_grp, model_type, model_name, y_pred, y_prob, y_train, x_train=x_train, x_test=x_test, \
                              model_object=model_object, pos_label=['TR','CR'], neg_label=['TN','CN'] ) 
...
def test_model_type_input():
    ...
    clf_obj._model_type_to_metric_lookup[clf_obj.model_params[0].model_type] = ('classification', 2, 2)
    with pytest.raises(MyError) as toolkit_exit:
        clf_obj._model_type_input()
    assert toolkit_exit.type == MyError
    
    if clf_obj.model_params[0].model_type == 'uplift' and clf_obj.model_params[1].model_name != "clone" :
...
def test_rootcause_group_difference():
    ...
    explainer_shap = shap.Explainer(clf_obj.model_params[0].model_object.predict_proba,x_train_sample) 
    explanation = explainer_shap(x_train_sample)
    shap_values = np.moveaxis(explanation.values, -1, 0)
    idx = list(clf_obj.model_params[0].model_object.classes_).index(clf_obj.model_params[0].pos_label[0])
...
def test_compute_correlation():
    ...
    assert bool(clf_obj.surrogate_features['isforeign']) == False