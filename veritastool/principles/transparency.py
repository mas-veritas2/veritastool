...
class Transparency:
    ...
    def __init__(self, tran_index = [1], tran_max_sample = 1, tran_pdp_feature = [], tran_pdp_target=None, tran_max_display = 10):
        ...
        self.permutation_importance = pd.DataFrame(columns = ["feature", "score"])
    ...
    def _local(self, n, model_num=0):
        ...
        if(n not in ind):
            ...
            'name': self.tran_processed_data.columns,
            ...
            other_features = pd.DataFrame({'name':str(len(plot_points[self.tran_max_display-1:])) + " OTHER",'value' : "" ,'shap': plot_points[self.tran_max_display-1:][['shap']].sum().values})
            if(plot_points.shape[0]==self.tran_max_display):
                local_plot_points = plot_points[:self.tran_max_display]
                feature_list = local_plot_points[['name','value', 'shap']]
            else:
            ...
            dict_item = {'id':n,'efx': efx,'fx':fx,'feature_info':feature_list[['name','value', 'shap']].to_dict(orient='records')}
    ...
    def _compute_permutation_importance(self,model_num=0):
        ...
        self.permutation_importance = pd.concat([self.permutation_importance, pd.DataFrame({'feature': new_list,'diff': diff})])
    ...
    def explain(self, local_index = None, output = True, model_num = None):
        ...
        if(self.model_params[model_num].model_type!='regression'):
            if(self.model_params[model_num].y_prob is None):
