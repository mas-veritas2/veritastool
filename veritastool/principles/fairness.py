...
class Fairness:
    ...
    def compile(self, disable=[], n_threads=1):
        ...
        self.compile_disable = exp_disable if disable is None else disable
        ...
        #check if evaluate hasn't run, only run if haven't
        if self.evaluate_status == 0 and 'evaluate' not in self.compile_disable:
            self.evaluate(visualize=False, output=False, n_threads=n_threads)
        #if evaluate is in disable, print skipped
        elif self.evaluate_status == 0 and 'evaluate' in self.compile_disable:
        ...
        elif self.tradeoff_status == 0 and 'tradeoff' not in self.compile_disable and 'evaluate' not in self.compile_disable:
            try :
        ...
        elif self.feature_imp_status == 0 and 'feature_importance' not in self.compile_disable and 'evaluate' not in self.compile_disable:
            try :
    ...
    def _generate_model_artifact(self):
        ...
        if 'evaluate' not in self.compile_disable:
            artifact_fair = {}
